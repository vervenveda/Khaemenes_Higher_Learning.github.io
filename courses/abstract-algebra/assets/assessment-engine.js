(() => {
"use strict";
const cfg=window.ABSTRACT_ALGEBRA_ASSESSMENT_CONFIG||{};
const all=cfg.source==="diagnostic"?window.ABSTRACT_ALGEBRA_DIAGNOSTIC:window.ABSTRACT_ALGEBRA_QUESTIONS;
const host=document.getElementById("assessmentRoot");
if(!host||!Array.isArray(all))return;
const KEY="khaemenes-abstract-algebra-assessment-v1";
...
function start(){
 ...
 const token=`abstract-algebra-${Date.now()}`;
