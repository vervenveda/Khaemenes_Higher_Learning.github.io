(() => {
"use strict";
const D=window.DIFFERENTIAL_EQUATIONS_DATA;
const Q=window.DIFFERENTIAL_EQUATIONS_QUESTIONS;
const $=(s,r=document)=>r.querySelector(s);
const $$=(s,r=document)=>[...r.querySelectorAll(s)];
const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const KEY=D.course.storage_key;
const PIN_KEY="khaemenes-higher-learning-pinned-courses-v1";
const COURSE_ID="differential-equations";
...
$("#pinCourseButton")?.addEventListener("click",()=>{
 ...
 if(status)status.textContent=on?"Differential Equations was removed from the Higher Learning profile.":"Differential Equations was pinned for the Higher Learning profile.";
});
renderPin();
...
function bindStudentControls(){
 ...
 $("#deleteLearner")?.addEventListener("click",()=>{
   ...
   if(confirm("Delete this learner's local Differential Equations record?")){
...
function renderDashboard(){
 ...
 <div class="section-head"><p class="eyebrow">Local learner dashboard</p><h2>${esc(s.name)} · Differential Equations</h2>
...
function renderAssessments(){
 ...
 <div class="resource-grid">
  ...
  <a class="card" href="assessments/final.html"><h3>Comprehensive Final</h3><p>100 questions across the complete Differential Equations course.</p></a>
...
function renderPortfolio(){
 ...
 $("#portfolioJson")?.addEventListener("click",()=>download("differential-equations-portfolio.json",JSON.stringify({course:D.course,learner:s,exported:new Date().toISOString()},null,2),"application/json"));$("#printPortfolio")?.addEventListener("click",()=>print());
}
function renderTeacher(){
 ...
 <article class="card col6"><h3>Scientific Calculator v4</h3><p>Use for exponentials, logarithms, trigonometry, complex roots, transforms, numerical checks, and parameter calculations while preserving equations, conditions, units, and interpretation.</p><a class="btn primary" href="https://vervenveda.com/Khaemenes_High.github.io/courses/mathematics/tools/calculator" target="_blank" rel="noopener">Open Calculator</a></article><article class="card col6"><h3>OHMIC CAD &amp; Geometry Arcade</h3><p>Use technical drafting, mechanical geometry, dimensions, components, and engineering context as documented modelling support—not as a substitute for differential-equation reasoning.</p><div class="actions"><a class="btn primary" href="https://vervenveda.com/proresource_hub.github.io/Protools/Ohmic_CAD_Studio.html" target="_blank" rel="noopener">Open CAD</a><a class="btn" href="https://vervenveda.com/arcade.github.io/a_sacred_geometry_game_index.html" target="_blank" rel="noopener">Open Sacred Geometry Studio</a></div></article>
 <article class="card col6"><h3>Profile integration</h3><p>This course stores the Higher Learning profile ID <code>differential-equations</code>. The Higher Learning portal can read this course ID in its future shared learner profile.</p><a class="btn" href="PROFILE_INTEGRATION_NOTES.md">Read Integration Notes</a></article>
 </div></div>`;
 bindStudentControls();
 $("#backup")?.addEventListener("click",()=>download("differential-equations-complete-backup.json",JSON.stringify(state,null,2),"application/json"));
 $("#import")?.addEventListener("change",async e=>{
   ...
   catch{alert("That file is not a valid Differential Equations backup.")}
 });
...
function downloadScope(){const rows=[["Week","Unit","Focus","Essential Question","Route"],...D.weeks.map(w=>[w.week,w.unit||"Milestone",w.title,w.essential,w.unit_path])];download("differential-equations-36-week-scope.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadGradebook(){const rows=[["Week","Focus","Classwork","Mastery Best","Portfolio","Sessions"],...D.weeks.map(w=>{const r=weekRecord(w.week);return [w.week,w.title,r.classwork??"",r.masteryBest??"",r.portfolio??"",Object.values(r.days).filter(Boolean).length]})];download("differential-equations-gradebook.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadReport(){
 const s=activeStudent(),g=courseGrade();
 const body=`<!doctype html><meta charset="utf-8"><title>Differential Equations Progress Report</title><style>body{font:16px Arial;max-width:1000px;margin:40px auto}table{border-collapse:collapse;width:100%}td,th{border:1px solid #999;padding:7px;text-align:left}</style><h1>${esc(s.name)} · Differential Equations Progress Report</h1><p>Exported ${new Date().toLocaleString()}</p><p>Pathway: ${esc(s.pathway)} · Calculated course grade: ${pct(g)}</p><p>Diagnostic ${pct(s.formal.diagnostic)} · Midterm ${pct(s.formal.midterm)} · Final ${pct(s.formal.final)} · Capstone ${pct(s.formal.capstone)}</p><table><thead><tr><th>Week</th><th>Focus</th><th>Classwork</th><th>Mastery</th><th>Portfolio</th></tr></thead><tbody>${D.weeks.map(w=>{const r=weekRecord(w.week);return `<tr><td>${w.week}</td><td>${esc(w.title)}</td><td>${pct(r.classwork)}</td><td>${pct(r.masteryBest)}</td><td>${pct(r.portfolio)}</td></tr>`}).join("")}</tbody></table>`;
 download("differential-equations-progress-report.html",body,"text/html");
}
