(() => {
"use strict";
const D=window.INTRODUCTION_TO_PROOFS_DATA;
const Q=window.INTRODUCTION_TO_PROOFS_QUESTIONS;
const $=(s,r=document)=>r.querySelector(s);
const $$=(s,r=document)=>[...r.querySelectorAll(s)];
const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const KEY=D.course.storage_key;
const PIN_KEY="khaemenes-higher-learning-pinned-courses-v1";
const COURSE_ID="introduction-to-proofs";
...
$("#pinCourseButton")?.addEventListener("click",()=>{
 ...
 if(status)status.textContent=on?"Introduction to Proofs was removed from the Higher Learning profile.":"Introduction to Proofs was pinned for the Higher Learning profile.";
});
renderPin();
...
function bindStudentControls(){
 ...
 $("#deleteLearner")?.addEventListener("click",()=>{
   ...
   if(confirm("Delete this learner's local Introduction to Proofs record?")){
...
function renderDashboard(){
 ...
 <div class="section-head"><p class="eyebrow">Local learner dashboard</p><h2>${esc(s.name)} · Introduction to Proofs</h2>
 ...
 <div class="grid">
  <article class="card col4"><h3>Learner Record</h3>${studentControls()}<p class="notice">Prerequisite: Algebra fluency and willingness to read and write mathematical arguments. Calculus or Discrete Mathematics is recommended but not required.. Placement is based on mastery, not age or grade.</p></article>
...
function renderUnits(){
 ...
 <div class="unit-grid"><article class="card unit-card" style="--accent:#111"><span class="unit-number">D</span><h3>Introduction to Proofs Readiness Diagnostic</h3><p>Forty placement questions in logic, sets, algebra, definitions, quantifiers, proof language, induction, and mathematical writing. Use results for placement and support, not exclusion.</p><div class="actions"><a class="btn primary" href="diagnostic/">Open Diagnostic</a></div></article>
...
function renderAssessments(){
 ...
 <div class="resource-grid">
  ...
  <a class="card" href="assessments/final.html"><h3>Comprehensive Final</h3><p>100 questions across the complete Introduction to Proofs course.</p></a>
...
function renderPortfolio(){
 ...
 $("#portfolioJson")?.addEventListener("click",()=>download("introduction to proofs-portfolio.json",JSON.stringify({course:D.course,learner:s,exported:new Date().toISOString()},null,2),"application/json"));$("#printPortfolio")?.addEventListener("click",()=>print());
}
function renderTeacher(){
 ...
 <article class="card col6"><h3>Profile integration</h3><p>This course stores the Higher Learning profile ID <code>introduction to proofs</code>. The Higher Learning portal can read this course ID in its future shared learner profile.</p><a class="btn" href="PROFILE_INTEGRATION_NOTES.md">Read Integration Notes</a></article>
 </div></div>`;
 bindStudentControls();
 $("#backup")?.addEventListener("click",()=>download("introduction to proofs-complete-backup.json",JSON.stringify(state,null,2),"application/json"));
 $("#import")?.addEventListener("change",async e=>{
   ...
   catch{alert("That file is not a valid Introduction to Proofs backup.")}
 });
...
function downloadScope(){const rows=[["Week","Unit","Focus","Essential Question","Route"],...D.weeks.map(w=>[w.week,w.unit||"Milestone",w.title,w.essential,w.unit_path])];download("introduction to proofs-36-week-scope.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadGradebook(){const rows=[["Week","Focus","Classwork","Mastery Best","Portfolio","Sessions"],...D.weeks.map(w=>{const r=weekRecord(w.week);return [w.week,w.title,r.classwork??"",r.masteryBest??"",r.portfolio??"",Object.values(r.days).filter(Boolean).length]})];download("introduction to proofs-gradebook.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadReport(){
 const s=activeStudent(),g=courseGrade();
 const body=`<!doctype html><meta charset="utf-8"><title>Introduction to Proofs Progress Report</title><style>body{font:16px Arial;max-width:1000px;margin:40px auto}table{border-collapse:collapse;width:100%}td,th{border:1px solid #999;padding:7px;text-align:left}</style><h1>${esc(s.name)} · Introduction to Proofs Progress Report</h1><p>Exported ${new Date().toLocaleString()}</p><p>Pathway: ${esc(s.pathway)} · Calculated course grade: ${pct(g)}</p><p>Diagnostic ${pct(s.formal.diagnostic)} · Midterm ${pct(s.formal.midterm)} · Final ${pct(s.formal.final)} · Capstone ${pct(s.formal.capstone)}</p><table><thead><tr><th>Week</th><th>Focus</th><th>Classwork</th><th>Mastery</th><th>Portfolio</th></tr></thead><tbody>${D.weeks.map(w=>{const r=weekRecord(w.week);return `<tr><td>${w.week}</td><td>${esc(w.title)}</td><td>${pct(r.classwork)}</td><td>${pct(r.masteryBest)}</td><td>${pct(r.portfolio)}</td></tr>`}).join("")}</tbody></table>`;
 download("introduction to proofs-progress-report.html",body,"text/html");
}
