...
$("#pinCourseButton")?.addEventListener("click",()=>{
 ...
 if(status)status.textContent=on?"Calculus II was removed from the Higher Learning profile.":"Calculus II was pinned for the Higher Learning profile.";
});
renderPin();
...
function bindStudentControls(){
 ...
 $("#deleteLearner")?.addEventListener("click",()=>{
   ...
   if(confirm("Delete this learner's local Calculus II record?")){
...
function renderDashboard(){
 ...
 <div class="section-head"><p class="eyebrow">Local learner dashboard</p><h2>${esc(s.name)} · Calculus II</h2>
 ...
 <div class="grid">
  <article class="card col4"><h3>Learner Record</h3>${studentControls()}<p class="notice">Prerequisite: Calculus I readiness or equivalent demonstrated mastery. Placement is based on mastery, not age or grade.</p></article>
...
function dailyPlan(w){
 ...
 {day:"Session 3",title:"Guided practice & representation",task:"Connect symbolic, graphical, numerical, and contextual representations.",work:["Complete pathway practice.","Use technology only after setting up the mathematics.","Record units, assumptions, and restrictions."]},
 {day:"Session 4",title:"Application, error analysis & modelling",task:"Apply the week’s ideas to a defensible context and analyze a common misconception.",work:["Identify the first invalid step in an error.","Correct it with a rule or counterexample.","State one model limitation."]},
...
function renderUnits(){
 ...
 <div class="unit-grid"><article class="card unit-card" style="--accent:#111"><span class="unit-number">D</span><h3>Calculus Readiness Diagnostic</h3><p>Forty prerequisite questions with explanations and targeted review categories.</p><div class="actions"><a class="btn primary" href="diagnostic/">Open Diagnostic</a></div></article>
...
function renderAssessments(){
 ...
 <div class="resource-grid">
  ...
  <a class="card" href="assessments/midterm.html"><h3>Midterm Examination</h3><p>60 questions covering Units 01–06: functions, polynomial and rational models, exponentials, logarithms, the unit circle, graphs, identities, and equations.</p></a>
  <a class="card" href="assessments/final.html"><h3>Comprehensive Final</h3><p>100 questions across the complete Calculus II course.</p></a>
...
function renderPortfolio(){
 ...
 $("#portfolioJson")?.addEventListener("click",()=>download("calculus-ii-portfolio.json",JSON.stringify({course:D.course,learner:s,exported:new Date().toISOString()},null,2),"application/json"));$("#printPortfolio")?.addEventListener("click",()=>print());
}
function renderTeacher(){
 ...
 <article class="card col6"><h3>Scientific Calculator v4</h3><p>Use for functions, logarithms, radians, trigonometric graphs and equations, vectors, polar and parametric models, matrices, sequences, limits, and verification while preserving formulas, domains, units, and interpretation.</p><a class="btn primary" href="https://vervenveda.com/Khaemenes_High.github.io/courses/mathematics/tools/calculator/" target="_blank" rel="noopener">Open Calculator</a></article><article class="card col6"><h3>OHMIC CAD &amp; Geometry Arcade</h3><p>Use technical drafting, bearings, vector components, radial design, polar patterns, rotation, and coordinate games as documented mathematical laboratories—not unrecorded extras.</p><div class="actions"><a class="btn primary" href="https://vervenveda.com/proresource_hub.github.io/Protools/Ohmic_CAD_Studio.html" target="_blank" rel="noopener">Open CAD</a><a class="btn" href="https://vervenveda.com/arcade.github.io/a_sacred_geometry_game_index.html" target="_blank" rel="noopener">Open Sacred Geometry Studio</a></div></article>
 <article class="card col6"><h3>Profile integration</h3><p>This course stores the Higher Learning profile ID <code>calculus-ii</code>. The Higher Learning portal can read this course ID in its future shared learner profile.</p><a class="btn" href="PROFILE_INTEGRATION_NOTES.md">Read Integration Notes</a></article>
 </div></div>`;
 bindStudentControls();
 $("#backup")?.addEventListener("click",()=>download("calculus-ii-complete-backup.json",JSON.stringify(state,null,2),"application/json"));
 $("#import")?.addEventListener("change",async e=>{
   ...
   catch{alert("That file is not a valid Calculus II backup.")}
 });
...
function downloadScope(){const rows=[["Week","Unit","Focus","Essential Question","Route"],...D.weeks.map(w=>[w.week,w.unit||"Milestone",w.title,w.essential,w.unit_path])];download("calculus-ii-36-week-scope.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadGradebook(){const rows=[["Week","Focus","Classwork","Mastery Best","Portfolio","Sessions"],...D.weeks.map(w=>{const r=weekRecord(w.week);return [w.week,w.title,r.classwork??"",r.masteryBest??"",r.portfolio??"",Object.values(r.days).filter(Boolean).length]})];download("calculus-ii-gradebook.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadReport(){
 const s=activeStudent(),g=courseGrade();
 const body=`<!doctype html><meta charset="utf-8"><title>Calculus II Progress Report</title><style>body{font:16px Arial;max-width:1000px;margin:40px auto}table{border-collapse:collapse;width:100%}td,th{border:1px solid #999;padding:7px;text-align:left}</style><h1>${esc(s.name)} · Calculus II Progress Report</h1><p>Exported ${new Date().toLocaleString()}</p><p>Pathway: ${esc(s.pathway)} · Calculated course grade: ${pct(g)}</p><p>Diagnostic ${pct(s.formal.diagnostic)} · Midterm ${pct(s.formal.midterm)} · Final ${pct(s.formal.final)} · Capstone ${pct(s.formal.capstone)}</p><table><thead><tr><th>Week</th><th>Focus</th><th>Classwork</th><th>Mastery</th><th>Portfolio</th></tr></thead><tbody>${D.weeks.map(w=>{const r=weekRecord(w.week);return `<tr><td>${w.week}</td><td>${esc(w.title)}</td><td>${pct(r.classwork)}</td><td>${pct(r.masteryBest)}</td><td>${pct(r.portfolio)}</td></tr>`}).join("")}</tbody></table>`;
 download("calculus-ii-progress-report.html",body,"text/html");
}
