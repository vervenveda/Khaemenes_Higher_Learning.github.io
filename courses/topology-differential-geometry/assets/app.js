(() => {
"use strict";
const D=window.TOPOLOGY_DIFFERENTIAL_GEOMETRY_DATA;
const Q=window.TOPOLOGY_DIFFERENTIAL_GEOMETRY_QUESTIONS;
const $=(s,r=document)=>r.querySelector(s);
const $$=(s,r=document)=>[...r.querySelectorAll(s)];
const esc=v=>String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const KEY=D.course.storage_key;
const PIN_KEY="khaemenes-higher-learning-pinned-courses-v1";
const COURSE_ID="topology-differential-geometry";
...
$("#pinCourseButton")?.addEventListener("click",()=>{
 ...
 if(status)status.textContent=on?"Topology & Differential Geometry was removed from the Higher Learning profile.":"Topology & Differential Geometry was pinned for the Higher Learning profile.";
});
renderPin();
...
function bindStudentControls(){
 ...
 $("#deleteLearner")?.addEventListener("click",()=>{
   ...
   if(confirm("Delete this learner's local Topology & Differential Geometry record?")){
...
function renderDashboard(){
 ...
 <div class="section-head"><p class="eyebrow">Local learner dashboard</p><h2>${esc(s.name)} · Topology & Differential Geometry</h2>
 ...
 <div class="grid">
  <article class="card col4"><h3>Learner Record</h3>${studentControls()}<p class="notice">Prerequisite: Introduction to Proofs or equivalent proof fluency, Real Analysis, multivariable calculus, and Linear Algebra are required. Abstract Algebra is strongly recommended for fundamental groups, homology, and bundle theory. Placement is based on mastery, not age or grade.</p></article>
  ...
  <article class="card col4"><h3>92 Printable Analytical Lessons</h3><p>Topic-specific definitions, theorem maps, proof structures, analytical diagrams, worked examples, six-problem printable worksheets, differentiated pathways, misconceptions, applications, reflection, and local completion.</p><button class="btn" data-view="units">Browse Units</button></article>
  <article class="card col4"><h3>Scientific Calculator v4</h3><p>Use the connected calculator for exact arithmetic, sequence values, complex arithmetic, combinatorics, numerical checks, history, and verification. Proof and theorem reasoning remain required.</p><a class="btn primary" href="https://vervenveda.com/Khaemenes_High.github.io/courses/mathematics/tools/calculator" target="_blank" rel="noopener">Open Calculator</a></article>
...
function dailyPlan(w){
 ...
 {day:"Session 4",title:"Application, proof critique & research connection",task:"Apply the week’s ideas to an advanced mathematical context and analyze a flawed proof, estimate, theorem application, or research claim.",work:["Identify the first invalid step in an error.","Correct it with a rule or counterexample.","State one limitation, boundary case, or unresolved question."]},
...
function renderUnits(){
 ...
 $("#view-units").innerHTML=`<div class="wrap"><div class="section-head"><p class="eyebrow">Deep instructional layer</p><h2>Diagnostic &amp; 13 Advanced Units</h2><p>Every unit contains complete lessons, Foundation/Core/Research / Graduate practice, a mastery check, applied investigation, learner guide, faculty guide, vocabulary, and standards map.</p></div>
 <div class="unit-grid"><article class="card unit-card" style="--accent:#111"><span class="unit-number">D</span><h3>Topology & Differential Geometry Readiness Diagnostic</h3><p>Forty placement questions in proof, sets, functions, metric spaces, real analysis, multivariable calculus, linear algebra, basic algebraic structures, curves, surfaces, and mathematical writing. Use results for placement and support, not exclusion.</p><div class="actions"><a class="btn primary" href="diagnostic/">Open Diagnostic</a></div></article>
...
function renderAssessments(){
 ...
 <div class="resource-grid">
  ...
  <a class="card" href="assessments/final.html"><h3>Comprehensive Final</h3><p>100 questions across the complete Topology & Differential Geometry course.</p></a>
...
function renderPortfolio(){
 ...
 $("#portfolioJson")?.addEventListener("click",()=>download("topology & differential geometry-portfolio.json",JSON.stringify({course:D.course,learner:s,exported:new Date().toISOString()},null,2),"application/json"));$("#printPortfolio")?.addEventListener("click",()=>print());
}
function renderTeacher(){
 ...
 <article class="card col6"><h3>Scientific Calculator v4</h3><p>Use for exact arithmetic, coordinate changes, metric coefficients, curvature values, Euler characteristic, homology ranks, numerical checks, and verification while preserving definitions and theorem hypotheses.</p><a class="btn primary" href="https://vervenveda.com/Khaemenes_High.github.io/courses/mathematics/tools/calculator" target="_blank" rel="noopener">Open Calculator</a></article><article class="card col6"><h3>OHMIC CAD &amp; Geometry Arcade</h3><p>Use quotient diagrams, cell complexes, manifold atlases, tangent frames, surface meshes, bundle maps, geodesics, and technical context as documented support—not as a substitute for proof.</p><div class="actions"><a class="btn primary" href="https://vervenveda.com/proresource_hub.github.io/Protools/Ohmic_CAD_Studio.html" target="_blank" rel="noopener">Open CAD</a><a class="btn" href="https://vervenveda.com/arcade.github.io/a_sacred_geometry_game_index.html" target="_blank" rel="noopener">Open Sacred Geometry Studio</a></div></article>
 <article class="card col6"><h3>Profile integration</h3><p>This course stores the Higher Learning profile ID <code>topology & differential geometry</code>. The Higher Learning portal can read this course ID in its future shared learner profile.</p><a class="btn" href="PROFILE_INTEGRATION_NOTES.md">Read Integration Notes</a></article>
 </div></div>`;
 bindStudentControls();
 $("#backup")?.addEventListener("click",()=>download("topology & differential geometry-complete-backup.json",JSON.stringify(state,null,2),"application/json"));
 $("#import")?.addEventListener("change",async e=>{
   ...
   catch{alert("That file is not a valid Topology & Differential Geometry backup.")}
 });
...
function downloadScope(){const rows=[["Week","Unit","Focus","Essential Question","Route"],...D.weeks.map(w=>[w.week,w.unit||"Milestone",w.title,w.essential,w.unit_path])];download("topology & differential geometry-36-week-scope.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadGradebook(){const rows=[["Week","Focus","Classwork","Mastery Best","Portfolio","Sessions"],...D.weeks.map(w=>{const r=weekRecord(w.week);return [w.week,w.title,r.classwork??"",r.masteryBest??"",r.portfolio??"",Object.values(r.days).filter(Boolean).length]})];download("topology & differential geometry-gradebook.csv",rows.map(r=>r.map(csvCell).join(",")).join("\n"),"text/csv")}
function downloadReport(){
 const s=activeStudent(),g=courseGrade();
 const body=`<!doctype html><meta charset="utf-8"><title>Topology & Differential Geometry Progress Report</title><style>body{font:16px Arial;max-width:1000px;margin:40px auto}table{border-collapse:collapse;width:100%}td,th{border:1px solid #999;padding:7px;text-align:left}</style><h1>${esc(s.name)} · Topology & Differential Geometry Progress Report</h1><p>Exported ${new Date().toLocaleString()}</p><p>Pathway: ${esc(s.pathway)} · Calculated course grade: ${pct(g)}</p><p>Diagnostic ${pct(s.formal.diagnostic)} · Midterm ${pct(s.formal.midterm)} · Final ${pct(s.formal.final)} · Capstone ${pct(s.formal.capstone)}</p><table><thead><tr><th>Week</th><th>Focus</th><th>Classwork</th><th>Mastery</th><th>Portfolio</th></tr></thead><tbody>${D.weeks.map(w=>{const r=weekRecord(w.week);return `<tr><td>${w.week}</td><td>${esc(w.title)}</td><td>${pct(r.classwork)}</td><td>${pct(r.masteryBest)}</td><td>${pct(r.portfolio)}</td></tr>`}).join("")}</tbody></table>`;
 download("topology & differential geometry-progress-report.html",body,"text/html");
}
